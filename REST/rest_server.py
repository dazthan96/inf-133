...
class RESTRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/lista_estudiantes":
            ...
            self.send_header("Content-type","application/json")
            self.end_headers()
            nombre =[estudiante["nombre"] for estudiante in estudiantes if (estudiante["nombre"].upper()).startswith("P")]
            self.wfile.write(json.dumps({"nombres que empiezan con P: ":nombre}).encode("utf-8"))
        elif self.path=="/contar_carrera":
            carreras=[]
            ...
            for i in range(len(carreras)):
                if not carreras[i] in lista_carreras:
                    lista_carreras[carreras[i]]=carreras.count(carreras[i])
            self.send_response(200)
            self.send_header("Content-type","application/json")
            self.end_headers()              
            self.wfile.write(json.dumps(lista_carreras).encode("utf-8"))
